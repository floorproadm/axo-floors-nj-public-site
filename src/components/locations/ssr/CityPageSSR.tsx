...
const defaultHeroImage = defaultHeroAsset.url;
...
/**
 * SSR-renderable copy of CityServiceAreaPage. Identical visual design;
 * differences are mechanical only:
 *   - Uses SafeLink (<a href>) instead of react-router-dom <Link>.
 *   - Uses HeaderSSR / FooterSSR (no router context).
 *   - Does NOT mount SEOHead or LocationStructuredData — metadata,
 *     canonical, robots and BreadcrumbList + Service JSON-LD are emitted
 *     by the TanStack route's head().
 *   - FAQ uses native <details> instead of Radix Accordion so the full
 *     content is in the server HTML and remains accessible without JS.
 */
...
const verifiedTestimonials = [
  { name: "Michelle Allen", text: "Outstanding experience overall! The workmanship was superb, we highly recommend AXO Floors to everyone!", location: "Ocean County, NJ" },
  { name: "Richard Davis", text: "We hired AXO Floors and they did amazing work! It was done fast, we had all the work completed. We would highly recommend to anyone! Thank you once again!", location: "Monmouth County, NJ" },
  { name: "David Nakano", text: "Professional and quality work! They transformed our home and we couldn't be happier with the results.", location: "Middlesex County, NJ" },
];
...
const decisionCards = [
  { icon: Sparkles, title: "Maintenance Recoat", bestFor: ["Surface wear", "Light scratches", "Finish that looks dull", "Floors without exposed raw wood or deep damage"], note: "Eligibility is confirmed during the on-site estimate.", href: "/refinishing" },
  { icon: Layers, title: "Full Sanding & Refinishing", bestFor: ["Widespread scratches", "Worn finish", "Outdated stain color", "Uneven appearance", "Floors requiring a complete finish reset"], href: "/refinishing" },
  { icon: Wrench, title: "Repairs & Refinishing", bestFor: ["Damaged boards", "Pet-stained sections", "Removed walls or altered room layouts", "Missing flooring", "Areas needing weave-in repair"], href: "/refinishing" },
  { icon: HomeIcon, title: "New Hardwood Installation", bestFor: ["Additions and new rooms", "Carpet replacement", "Significant floor replacement", "Solid or engineered hardwood selection"], href: "/installation" },
];
...
const buildFaq = (cityName: string, custom?: { q: string; a: string }[]) => {
  ...
  return [
    { q: `Do you provide hardwood floor estimates in ${cityName}, NJ?`, a: `Yes. AXO Floors provides free, no-pressure in-home estimates in ${cityName}. You can request one online or call ${AXO_PHONE_DISPLAY}.` },
    { q: "Can my floors be refinished or do they need replacement?", a: "It depends on remaining board thickness, the existing finish, and damage. During the estimate we evaluate each room and explain whether refinishing, isolated repair, or replacement is the most appropriate option." },
    { q: "Is sanding completely dust-free?", a: "No process is 100% dust-free, but we use dust-contained sanding equipment that captures the large majority of dust at the source, keeping your home cleaner during the work." },
    { q: "Can engineered hardwood be refinished?", a: "Often yes, depending on wear-layer thickness. Thicker wear layers can typically be sanded once or twice; very thin layers may be better suited to a recoat rather than a full sand." },
    { q: "Can damaged or pet-stained boards be repaired?", a: "Yes. We weave in replacement boards and blend the new pieces with the surrounding floor during sanding and finishing. Severely stained areas are evaluated for repair versus replacement on-site." },
    { q: "How long does a typical hardwood project take?", a: "Project duration depends on square footage, repairs, stain selection, finish system, and drying and curing requirements. We share a realistic timeline after the on-site assessment." },
    { q: "Can I stay in the home during the project?", a: "Many homeowners stay in part of the home during the work, but it depends on layout, finish system, and curing time. We discuss living arrangements during the estimate." },
    { q: "What is the difference between water-based and oil-based finishes?", a: "Water-based finishes tend to be lower-odor and faster to cure, while oil-based finishes have a warmer amber tone and a different cure profile. The right choice depends on wood species, desired appearance, and household use." },
    { q: "When can furniture be returned to the room?", a: "Light foot traffic is typically possible well before furniture return. Heavy furniture and rugs require longer cure times that depend on the finish system used. We provide specific guidance after final coat." },
    { q: "Do you refinish stairs and railings?", a: "Yes. We refinish existing stair treads and risers, convert carpeted stairs to hardwood, and work on handrail systems — matched to first-floor stain where appropriate." },
  ];
};

const CityPageSSR = ({ location }: Props) => {
  ...
  return (
    <>
      ...
      <main className="bg-background">
          ...
          <div className="grid lg:grid-cols-2 gap-10 lg:gap-14 items-center">
            ...
            <div className="relative order-first lg:order-last">
              <div className="relative rounded-2xl overflow-hidden shadow-elegant border-4 border-gold/20 aspect-[4/3] lg:aspect-[5/4]">
                <img src={heroSrc} alt="Refinished red oak hardwood foyer by AXO Floors NJ" width={1200} height={900} className="w-full h-full object-cover" />
              </div>
            </div>
          </div>
        </section>
          ...
          <div className="container mx-auto px-4 py-14 md:py-20">
            ...
            <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
              {decisionCards.map((card) => {
                ...
                return (
                  <div key={card.title} className="flex flex-col rounded-xl border border-grey-light bg-card p-6 hover:border-gold hover:shadow-soft transition-smooth">
                    ...
                    <ul className="space-y-1.5 text-sm text-grey leading-relaxed mb-4">
                      {card.bestFor.map((b) => (
                        <li key={b} className="flex gap-2"><span className="text-gold">·</span><span>{b}</span></li>
                      ))}
                    </ul>
                    {card.note && <p className="text-xs text-grey/80 italic mb-4">{card.note}</p>}
                    <SafeLink to={card.href} className="mt-auto inline-flex items-center text-sm font-semibold text-navy hover:text-gold transition-smooth">
          ...
          <div className="container mx-auto px-4 py-14 md:py-20">
            <div className="grid lg:grid-cols-2 gap-10 lg:gap-14 items-center">
              <div className="rounded-2xl overflow-hidden border-4 border-gold/20 shadow-elegant aspect-[4/5] max-w-md mx-auto lg:mx-0">
                <img src={teamPhoto} alt="Ademir and Eduardo — AXO Floors family team" loading="lazy" width={800} height={1000} className="w-full h-full object-cover object-top" />
              </div>
              <div className="min-w-0 max-w-prose">
                ...
                <Button asChild variant="outline" className="border-gold text-gold hover:bg-gold hover:text-black">
                  <a href="/about">Read the full AXO Floors story</a>
                </Button>
              </div>
            </div>
          </div>
        ...
        <section className="container mx-auto px-4 py-14 md:py-20">
          ...
          <ol className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {processSteps.map((s, i) => {
              const Icon = s.icon;
              return (
                <li key={s.title} className="rounded-xl border border-grey-light bg-card p-6 hover:border-gold transition-smooth">
                  <div className="flex items-center justify-between mb-4">
                    <div className="w-10 h-10 rounded-lg bg-gold/10 flex items-center justify-center">
                      <Icon className="h-5 w-5 text-gold" aria-hidden="true" />
                    </div>
                    <span className="text-2xl font-bold font-heading text-navy/15">0{i + 1}</span>
                  </div>
        ...
        <section className="bg-grey-light/40 border-y border-grey-light">
          <div className="container mx-auto px-4 py-14 md:py-20">
            ...
            <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
              {[
                { icon: Palette, title: "Color: natural vs. stained", body: "Natural floors highlight the wood's own tone. Stains shift the color — warmer, deeper, lighter, or grey — and the species of wood affects how stain takes." },
                { icon: Sparkles, title: "Sheen: matte to higher gloss", body: "Matte and satin finishes hide minor wear and feel modern. Higher-sheen options show more light and depth but also more imperfection." },
                { icon: Shield, title: "Water-based vs. oil-based", body: "Water-based systems cure faster and have lower odor. Oil-based systems offer a warmer amber tone and a different cure profile. Either may be appropriate depending on the project." },
              ].map((c) => {
                const Icon = c.icon;
                return (
                  <div key={c.title} className="rounded-xl border border-grey-light bg-card p-6">
          ...
          <div className="grid lg:grid-cols-3 gap-10">
            ...
            <aside className="space-y-6">
              {location.neighborhoods.length > 0 && (
                <div>
                  <h3 className="text-sm font-semibold uppercase tracking-wider text-gold mb-3">Areas Served</h3>
                  <ul className="flex flex-wrap gap-2">
                    {location.neighborhoods.map((n) => (
                      <li key={n} className="text-sm px-3 py-1.5 rounded-full bg-grey-light/60 text-navy">{n}</li>
                    ))}
                  </ul>
                </div>
              )}
              {location.zipCodes.length > 0 && (
                <div>
                  <h3 className="text-sm font-semibold uppercase tracking-wider text-gold mb-3">ZIP Codes</h3>
                  <ul className="flex flex-wrap gap-2">
                    {location.zipCodes.map((z) => (
                      <li key={z} className="text-sm px-3 py-1.5 rounded-full bg-grey-light/60 text-navy font-mono">{z}</li>
                    ))}
                  </ul>
                </div>
              )}
          ...
          <div className="container mx-auto px-4 py-14 md:py-20">
            ...
            <div className="grid gap-5 md:grid-cols-3">
              {verifiedTestimonials.map((t) => (
                <figure key={t.name} className="rounded-xl border border-grey-light bg-card p-6 flex flex-col">
                  <div className="flex gap-0.5 mb-3" aria-label="5 out of 5 stars">
                    {Array.from({ length: 5 }).map((_, i) => (
                      <Star key={i} className="h-4 w-4 fill-gold text-gold" aria-hidden="true" />
                    ))}
                  </div>
                  <blockquote className="text-grey leading-relaxed flex-1">"{t.text}"</blockquote>
          ...
          <section className="container mx-auto px-4 py-14 md:py-20">
            ...
            <ul className="flex flex-wrap gap-3">
              {publishedNearby.map((c) => (
                <li key={c.slug}>
                  <SafeLink to={`/service-areas/new-jersey/${c.slug}`} className="inline-flex items-center px-4 py-2 rounded-lg border border-grey-light text-navy font-medium hover:border-gold hover:bg-grey-light/30 transition-smooth">
        ...
        {/* FAQ — native <details> so the answers are in the server HTML */}
        <section className="container mx-auto px-4 py-14 md:py-20">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold font-heading text-navy mb-8">
            Frequently Asked Questions
          </h2>
          <div className="max-w-3xl divide-y divide-grey-light border-y border-grey-light">
            {faq.map((item, i) => (
              <details key={i} className="group py-4">
                <summary className="flex cursor-pointer items-center justify-between gap-4 text-left text-navy font-semibold text-base list-none [&::-webkit-details-marker]:hidden">
                  <span>{item.q}</span>
                  <ChevronRight className="h-4 w-4 shrink-0 transition-transform group-open:rotate-90" aria-hidden="true" />
                </summary>
                <div className="pt-3 text-grey leading-relaxed text-base">{item.a}</div>
              </details>
            ))}
          </div>
        </section>
